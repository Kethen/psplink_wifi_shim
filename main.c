...
static void _vsnprintf_callback(void *_ctx, int c){
	struct _vsnprintf_ctx *ctx = _ctx;
	if (c == 0x200){
		return;
	}
...
static int _vsnprintf(char *buf, int len, const char *fmt, va_list args){
	struct _vsnprintf_ctx ctx = {
		.max_len = len,
		.offset = 0,
		.buf = buf
	};

	prnt(_vsnprintf_callback, &ctx, fmt, args);
...
static void log_memory_info_wifi(){
	...
	for(int i = 1;i < 13;i++){
		...
		if (query_status == 0){
			int max_free = sceKernelPartitionMaxFreeMemSize(i);
			int total_free = sceKernelPartitionTotalFreeMemSize(i);
			len = sprintf(buf, "p%d startaddr 0x%x size %d attr 0x%x max %d total %d\n", i, meminfo.startaddr, meminfo.memsize, meminfo.attr, max_free, total_free);
		}else{
			len = sprintf(buf, "p%d query failed, 0x%x\n", i, query_status);
		}
		fill_buffer(buf, len);
	}
}
...
int send_input_to_psplink_thread_func(unsigned int args, void *arg){
	...
	while(1){
		...
		if (strcmp(to_psplink_input, "help") == 0){
			...
			len += sprintf(&buf[len], "%-10s - %s\n", "meminfo2", "meminfo with more partitions");
			len += sprintf(&buf[len], "%-10s - %s\n\n", "exit2", "runs sctrlKernelExitVSH(NULL) to reboot to VSH");
			fill_buffer(buf, len);
		}
